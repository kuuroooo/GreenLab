...
G3 OPTIMIZATION: Use loop optimization techniques (e.g., loop unrolling, loop 
unswitching, early termination) to improve performance, such as storing loop end 
...
class BodySystem:
    ...
    def update_system(self, delta_time: float) -> None:
        ...
        G3 OPTIMIZATION: Use triangular loop to avoid redundant calculations
        and store loop end conditions in variables.
        ...
        (0.1, 0.0)
        """
        # G3: Store loop end condition in variable
        bodies = self.bodies
        n_bodies = len(bodies)
        if n_bodies == 0:
            return
        ...
        # G3: Use triangular loop to avoid redundant calculations
        # Each pair (i,j) is calculated only once, then applied to both bodies
        for i in range(n_bodies - 1):
            body1 = bodies[i]
            body1_x = body1.position_x
            body1_y = body1.position_y
            body1_mass = body1.mass
            
            for j in range(i + 1, n_bodies):
                body2 = bodies[j]
                body2_x = body2.position_x
                body2_y = body2.position_y
                ...
                distance = (dif_x**2 + dif_y**2 + softening_factor) ** (1 / 2)

                # Newton's law of universal gravitation.
                force_magnitude = gravitation_constant / distance**3
                force_x = force_magnitude * dif_x
                force_y = force_magnitude * dif_y
                
                # Apply forces to both bodies (equal and opposite)
                body1.force_x = getattr(body1, 'force_x', 0.0) + force_x * body2_mass
                body1.force_y = getattr(body1, 'force_y', 0.0) + force_y * body2_mass
                body2.force_x = getattr(body2, 'force_x', 0.0) - force_x * body1_mass
                body2.force_y = getattr(body2, 'force_y', 0.0) - force_y * body1_mass

        # Update velocities and positions
        for body in bodies:
            force_x = getattr(body, 'force_x', 0.0)
            force_y = getattr(body, 'force_y', 0.0)
            body.update_velocity(force_x, force_y, dt)
            # Clear forces for next iteration
            delattr(body, 'force_x')
            delattr(body, 'force_y')
