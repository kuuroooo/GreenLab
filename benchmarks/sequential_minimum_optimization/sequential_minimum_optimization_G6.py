...
class SmoSVM:
    ...
    def _choose_a2(self, i1):
        ...
        self._unbound = [i for i in self._all_samples if self._is_unbound(i)]  # G6
        ...
        if len(self._unbound) > 0:
            ...
            tmp_error = self._error.copy().tolist()  # G6
            tmp_error_dict = {
                index: value
        ...
        for i2 in np.roll(self._unbound, rng.choice(self.length)):  # G6: Use already computed _unbound
            cmd = yield i1, i2
            if cmd is None:
                return
