...
class SmoSVM:
        ...
        while True:
            ...
            for s in self.unbound:
                if s in (i1, i2):
                    continue
                self._error[s] += (
                    d1 * k(i1, s) +  # G1: Use precomputed d1
                    d2 * k(i2, s) +  # G1: Use precomputed d2
                    (self._b - b_old)
                )
    ...
    def _choose_a2(self, i1):
        ...
        self._unbound = [i for i in self._all_samples if is_unbound(i)]

        if len(self.unbound) > 0:
            tmp_error = err.copy().tolist()  # G1: Use bound variable
            tmp_error_dict = {
                index: value
                for index, value in enumerate(tmp_error)
                if is_unbound(index)  # G1: Use bound variable
            }
            if self._e(i1) >= 0:
