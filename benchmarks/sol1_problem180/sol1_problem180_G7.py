...
def solution(order: int = 35) -> int:
    ...
    # G7: Use bulk operations - iterate over precomputed fractions
    for i, (x_num, x_den) in enumerate(fractions):
        ...
        for j, (y_num, y_den) in enumerate(fractions):
            ...
            # n=2
            z_num = xn2 * yd2 + xd2 * yn2  # G7: Use precomputed squares
            z_den = xd2 * yd2  # G7: Use precomputed squares
            ...
            # n=-2
            z_num = xn2 * yn2  # G7: Use precomputed squares
            z_den = xd2 * yn2 + xn2 * yd2  # G7: Use precomputed squares
            if z_num > 0 and z_den > 0:
                z_num_sqrt = isqrt(z_num)
                z_den_sqrt = isqrt(z_den)
                if z_num_sqrt * z_num_sqrt == z_num and z_den_sqrt * z_den_sqrt == z_den:
    ...
    # G7: Use bulk operations for final sum calculation
