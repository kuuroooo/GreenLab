...
def solution(order: int = 35) -> int:
    ...
    O = order  # Cache order value
    OP1 = O + 1  # Cache order + 1
    ...
    for x_num in range(1, OP1):
        for x_den in range(x_num + 1, OP1):
            # G1: Precompute x squares to avoid repeated multiplication
            xn2 = x_num * x_num
            xd2 = x_den * x_den
            for y_num in range(1, OP1):
                for y_den in range(y_num + 1, OP1):
                    # G1: Precompute y squares to avoid repeated multiplication
                    yn2 = y_num * y_num
                    yd2 = y_den * y_den
