...
def solution(order: int = 35) -> int:
    ...
    # G3: Precompute loop bounds to avoid repeated calculations
    x_num_end = OP1
    for x_num in range(1, x_num_end):
        x_den_start = x_num + 1
        x_den_end = OP1
        for x_den in range(x_den_start, x_den_end):
            # G3: Precompute squares once per x values
            xn2 = x_num * x_num
            xd2 = x_den * x_den
            
            y_num_end = OP1
            for y_num in range(1, y_num_end):
                y_den_start = y_num + 1
                y_den_end = OP1
                for y_den in range(y_den_start, y_den_end):
                    # G3: Precompute squares once per y values
                    yn2 = y_num * y_num
                    yd2 = y_den * y_den
                    ...
                    z_num //= hcf
                    z_den //= hcf
                    # G3: Early termination - check bounds first
                    if z_num <= 0 or z_den <= z_num or z_den > O:
                        continue
