# === Applied G7: Use bulk NumPy operations to reduce per-element overhead
# Comments mark every change related to G7.
import numpy as np  # APPLIED G7: use NumPy for bulk operations
"""
...
def main(
    ...
    pheromone = np.full((cities_num, cities_num), 1.0)  # APPLIED G7: initialize as NumPy array for bulk ops
...
def pheromone_update(
    ...
    KeyError: 0
    """
    # APPLIED G7: bulk multiply evaporation across entire matrix (replaces nested loops)
    ...
    for ant_route in ants_route:
        ...
        for i in range(len(ant_route) - 1):  # Update pheromones
            pheromone[ant_route[i], ant_route[i + 1]] += delta_pheromone  # APPLIED G7: NumPy indexing
            pheromone[ant_route[i + 1], ant_route[i]] = pheromone[ant_route[i], ant_route[i + 1]]  # APPLIED G7: NumPy symmetric update
