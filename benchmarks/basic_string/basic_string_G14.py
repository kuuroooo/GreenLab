...
def select(
    ...
    for _ in range(child_n):
        ...
        # G14: Use cached top slice instead of recomputing
        parent_2 = top_slice[random.randint(0, min(N_SELECTED, len(top_slice) - 1))][0]
...
def basic(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    def generate_individual():
        return "".join(random.choice(genes) for _ in range(cached_target_length))
