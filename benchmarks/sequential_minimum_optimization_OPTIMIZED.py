...
class SmoSVM:
    def __init__(
        ...
        # G14: Cache frequently used shared data
        self._support_idx = np.array([], dtype=int)
        self._ay_sv = np.array([], dtype=np.float64)
        self._support_dirty = True
        self._ay = self.alphas * self.tags  # G14: Cache ay vector
    ...
    def fit(self):
        ...
        while True:
            ...
            a1, a2 = self.alphas[i1], self.alphas[i2]  # G6: Remove unnecessary .copy() for scalars
            ...
            # G1: Hoist repeated expressions into locals
            k11 = k(i1, i1)  # G1
            k22 = k(i2, i2)  # G1
            k12 = k(i1, i2)  # G1 (symmetry: k21 == k12)
            d1 = y1 * (a1_new - a1)  # G1
            d2 = y2 * (a2_new - a2)  # G1
            
            # 3: update threshold(b)
            b1_new = np.float64(-e1 - d1 * k11 - d2 * k12 + self._b)  # G1
            b2_new = np.float64(-e2 - d2 * k22 - d1 * k12 + self._b)  # G1
            
            # G4: Use short-circuit logical operators
            if 0.0 < a1_new < self._c:  # G4
                b = b1_new
            elif 0.0 < a2_new < self._c:  # G4
                b = b2_new
            else:
                b = (b1_new + b2_new) / 2.0
            b_old = self._b
            ...
            # G14: Update ay cache incrementally
            self._ay[i1] += d1  # G14
            self._ay[i2] += d2  # G14
            self._support_dirty = True  # G14

            # G7: Vectorized error update for all unbound samples at once
            self._unbound = [i for i in self._all_samples if self._is_unbound(i)]
            if self._unbound:  # G7
                ub = np.array(self._unbound, dtype=int)
                self._error[ub] += (
                    d1 * self._K_matrix[i1, ub] +
                    d2 * self._K_matrix[i2, ub] +
                    (self._b - b_old)
                )
    ...
    def predict(self, test_samples, classify=True):
        ...
        # G14: Refresh support cache if dirty
        if self._support_dirty:  # G14
            self._refresh_support_cache()  # G14
        
        # G7: Batch prediction using support vectors only
        sv = self._support_idx  # G7
        if sv.size == 0:  # G7
            scores = np.full(test_samples.shape[0], self._b, dtype=np.float64)  # G7
        else:  # G7
            scores = self._bulk_kernel(self.samples[sv], test_samples) @ self._ay_sv + self._b  # G7
        
        return np.where(scores > 0, 1, -1) if classify else scores  # G7
    ...
    def _e(self, index):
        ...
        # get by g(xi) - yi
            ...
            # G14: Use cached ay vector to avoid recomputation
            gx = np.dot(self._ay, self._K_matrix[:, index]) + self._b  # G14
            yi = self.tags[index]  # G14
            return gx - yi  # G14

    # Calculate kernel matrix of all possible i1, i2, saving time
    def _calculate_k_matrix(self):
        # G3: Store loop bounds and exploit symmetry
        n = self.length  # G3
        k_matrix = np.zeros((n, n), dtype=np.float64)  # G3
        
        # G3: Exploit symmetry - fill only upper triangle and mirror
        for i in range(n):  # G3
            k_matrix[i, i] = self.Kernel(self.samples[i], self.samples[i])  # G3
            for j in range(i + 1, n):  # G3
                kij = self.Kernel(self.samples[i], self.samples[j])  # G3
                k_matrix[i, j] = kij  # G3
                k_matrix[j, i] = kij  # G3 (symmetry)
        return k_matrix
    ...
    def _choose_a2(self, i1):
        ...
        Steps:
            ...
            3: Start in a random point, loop over all samples till alpha1 and alpha2 are
               optimized.
        """
        # G1: Bind locals to avoid repeated attribute lookups
        is_unbound = self._is_unbound  # G1
        err = self._error  # G1
        
        # G6: Compute unbound only once and reuse
        self._unbound = [i for i in self._all_samples if is_unbound(i)]  # G6

        if len(self._unbound) > 0:
            # G6: Use the already computed _unbound list, convert to array only once
            ub = np.array(self._unbound, dtype=int)  # G6
            ub_err = err[ub]  # G6
            # G7: Use NumPy argmin/argmax for bulk operations
            if self._e(i1) >= 0:  # G7
                i2 = ub[np.argmin(ub_err)]  # G7
            else:  # G7
                i2 = ub[np.argmax(ub_err)]  # G7
            cmd = yield i1, i2
            if cmd is None:
                return

        # G3: Store loop bounds in variable
        n = self.length  # G3
        rng = np.random.default_rng()  # G3
        for i2 in np.roll(self._unbound, rng.choice(n)):  # G3
            cmd = yield i1, i2
            if cmd is None:
                return

        for i2 in np.roll(self._all_samples, rng.choice(n)):  # G3
            cmd = yield i1, i2
            if cmd is None:
                return
    ...
    # G14: Refresh support vector cache
    def _refresh_support_cache(self):
        sv = np.flatnonzero(self.alphas > 0.0)  # G14
        self._support_idx = sv  # G14
        self._ay_sv = self.alphas[sv] * self.tags[sv]  # G14
        self._support_dirty = False  # G14
    
    # G7: Bulk kernel computation for vectorized operations
    def _bulk_kernel(self, A, B):  # A: [m,d], B: [n,d] → returns [n,m] usable with @
        if self.Kernel._kernel_name == "linear":  # G7
            return (B @ A.T) + self.Kernel.coef0  # G7
        if self.Kernel._kernel_name == "poly":  # G7
            return (self.Kernel.gamma * (B @ A.T) + self.Kernel.coef0) ** self.Kernel.degree  # G7
        # rbf  # G7
        A2 = np.sum(A*A, axis=1)[None, :]  # G7
        B2 = np.sum(B*B, axis=1)[:, None]  # G7
        D2 = B2 + A2 - 2.0 * (B @ A.T)  # G7
        return np.exp(-self.Kernel.gamma * D2)  # G7
...
def plot_partition_boundary(
    ...
    # G3: Precompute loop bounds
    xrange = np.linspace(train_data_x.min(), train_data_x.max(), resolution)  # G3
    yrange = np.linspace(train_data_y.min(), train_data_y.max(), resolution)  # G3
    xlen, ylen = len(xrange), len(yrange)  # G3
    
    # G7: Generate grid in bulk rather than list comprehension
    xx, yy = np.meshgrid(xrange, yrange)  # G7
    test_samples = np.column_stack([xx.ravel(), yy.ravel()])  # G7

    # G7: Batch predict the entire grid at once
    test_tags = model.predict(test_samples, classify=False)  # G7
    grid = test_tags.reshape((ylen, xlen))  # G7

    # Plot contour map which represents the partition boundary
    ax.contour(
        xrange,
        yrange,
        grid,  # G7: No need for .T since we reshaped correctly
        levels=(-1, 0, 1),
        linestyles=("--", "-", "--"),
        linewidths=(1, 1, 1),
        colors=colors,
