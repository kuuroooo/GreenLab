...
def solution(order: int = 35) -> int:
    ...
    """
    unique_s: set = set()
    # G6: Remove unnecessary variables - only keep what's needed
    total: Fraction = Fraction(0)

    for x_num in range(1, order + 1):
        for x_den in range(x_num + 1, order + 1):
            # G6: Precompute squares once per x values to avoid recomputation
            xn2 = x_num * x_num
            xd2 = x_den * x_den
            for y_num in range(1, order + 1):
                for y_den in range(y_num + 1, order + 1):
                    # G6: Precompute squares once per y values to avoid recomputation
                    yn2 = y_num * y_num
                    yd2 = y_den * y_den
                    ...
                    z_den = x_den * y_den
                    # G6: Use gcd directly instead of storing in variable
                    g = gcd(z_num, z_den)
                    z_num //= g
                    z_den //= g
                    if 0 < z_num < z_den <= order:
                        # G6: Directly add to set instead of storing in intermediate variable
                        unique_s.add(add_three(
                            x_num, x_den, y_num, y_den, z_num, z_den
                        ))

                    # n=2
                    z_num = xn2 * yd2 + xd2 * yn2  # G6: Use precomputed squares
                    z_den = xd2 * yd2  # G6: Use precomputed squares
                    # G6: Use isqrt instead of is_sq + sqrt combination to avoid double computation
                    if z_num > 0 and z_den > 0:
                        z_num_sqrt = isqrt(z_num)
                        z_den_sqrt = isqrt(z_den)
                    ...
                    # n=-2
                    z_num = xn2 * yn2  # G6: Use precomputed squares
                    z_den = xd2 * yn2 + xn2 * yd2  # G6: Use precomputed squares
                    # G6: Use isqrt instead of is_sq + sqrt combination to avoid double computation
                    if z_num > 0 and z_den > 0:
                        z_num_sqrt = isqrt(z_num)
                        z_den_sqrt = isqrt(z_den)
