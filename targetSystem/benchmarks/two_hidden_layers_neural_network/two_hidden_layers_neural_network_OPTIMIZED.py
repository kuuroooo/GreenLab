...
# ALL applied: G1 + G3 + G6 + G7
# G1: common subexpression elimination in backprop (already done via variables).
# G3: early termination / cached loop-end in training.
# G6: remove redundant buffers and reuse computed outputs.
# G7: vectorized batch prediction.
...
class TwoHiddenLayerNeuralNetwork:
    ...
    def train(self, output: np.ndarray, iterations: int, give_loss: bool) -> None:
        """
        Training with early termination (G3) and reuse of computed outputs (G6).
        """
