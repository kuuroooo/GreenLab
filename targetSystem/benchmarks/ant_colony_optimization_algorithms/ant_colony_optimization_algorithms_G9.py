# === Applied G9: Precompute distance matrix to decouple per-ant work
# Comments mark every change related to G9.
"""
...
DIST_MATRIX = None  # APPLIED G9: module-level cache to satisfy linters and runtime

# APPLIED G9: precompute distance matrix once to reduce inter-iteration dependencies
def _precompute_distances(cities):
    # APPLIED G9: distance matrix so each ant's computation is independent
    n = len(cities)
    import math
...
def pheromone_update(
    ...
    for ant_route in ants_route:
        ...
        for i in range(len(ant_route) - 1):  # Calculate total distance
            global DIST_MATRIX  # APPLIED G9: use module-level cache
            if DIST_MATRIX is None:
                DIST_MATRIX = _precompute_distances(cities)  # APPLIED G9: cached once
            total_distance += DIST_MATRIX[ant_route[i]][ant_route[i + 1]]  # APPLIED G9
