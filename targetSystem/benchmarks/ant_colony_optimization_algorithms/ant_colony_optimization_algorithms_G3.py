# === Applied G3: Loop optimization: cached bounds and early termination
# Comments mark every change related to G3.
"""
...
def pheromone_update(
    ...
    KeyError: 0
    """
    cities_len = len(cities)  # APPLIED G3: store loop bound once
    for a in range(cities_len):  # APPLIED G3: use cached bound
                for b in range(cities_len):  # APPLIED G3: use cached bound
                    pheromone[a][b] *= pheromone_evaporation
    for ant_route in ants_route:
        total_distance = 0.0
        best_so_far = best_distance  # APPLIED G3: snapshot for early-termination comparison
        for i in range(len(ant_route) - 1):  # APPLIED G3: calculate total distance with early termination
            # APPLIED G3: early termination if partial sum already worse than best
