# === Applied G12: Enable dynamic scheduling with ThreadPoolExecutor
# Comments mark every change related to G12.
"""
...
import copy
import random
from concurrent.futures import ThreadPoolExecutor, as_completed  # APPLIED G12

# APPLIED G12: build a single-ant route function to submit as independent tasks
def _build_ant_route(cities, alpha, beta, pheromone):  # APPLIED G12: accept pheromone from caller
    # APPLIED G12
...
def main(
    ...
    for _ in range(iterations_num):
        ants_route = []
        # APPLIED G12: dynamic scheduling via ThreadPoolExecutor; idle workers steal tasks
        with ThreadPoolExecutor() as ex:
            futures = [ex.submit(_build_ant_route, cities, alpha, beta, pheromone) for _ in range(ants_num)]  # APPLIED G12
            for f in as_completed(futures):  # APPLIED G12: collect as tasks complete
                ants_route.append(f.result())
        # APPLIED G12: routes built in parallel; remove sequential construction
        # (city_select calls remain unchanged)
