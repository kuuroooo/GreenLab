...
class SmoSVM:
    ...
    def fit(self):
        ...
        while True:
            ...
            if self._unbound:  # G7
                ub = np.array(self._unbound, dtype=int)  # G7
                self._error[ub] += (
                    y1 * (a1_new - a1) * self._K_matrix[i1, ub] +  # G7
                    y2 * (a2_new - a2) * self._K_matrix[i2, ub] +  # G7
                    (self._b - b_old)  # G7
                )
    ...
    def predict(self, test_samples, classify=True):
        ...
        sv = np.flatnonzero(self.alphas > 0.0)  # G7
        if sv.size == 0:  # G7
            scores = np.full(test_samples.shape[0], self._b, dtype=np.float64)  # G7
        else:  # G7
            ay_sv = self.alphas[sv] * self.tags[sv]  # G7
            scores = self._bulk_kernel(self.samples[sv], test_samples) @ ay_sv + self._b  # G7
    ...
    def _choose_a2(self, i1):
        ...
        if len(self.unbound) > 0:
            ...
            if self._e(i1) >= 0:  # G7
                i2 = min(tmp_error_dict, key=lambda index: tmp_error_dict[index])  # G7
            else:  # G7
                i2 = max(tmp_error_dict, key=lambda index: tmp_error_dict[index])  # G7
            cmd = yield i1, i2
            if cmd is None:
                return
...
def plot_partition_boundary(
    ...
    xrange = np.linspace(train_data_x.min(), train_data_x.max(), resolution)  # G7
    yrange = np.linspace(train_data_y.min(), train_data_y.max(), resolution)  # G7
    ...
    grid = test_tags.reshape((len(xrange), len(yrange)))  # G7
