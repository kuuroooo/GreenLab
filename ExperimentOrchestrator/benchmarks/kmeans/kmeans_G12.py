...
# G12: enable parallel scheduling using joblib for centroid updates
def revise_centroids(data, k, cluster_assignment):
    # [G12 applied here: parallel computation of centroids]
    def compute_centroid(i):
        member_data_points = data[cluster_assignment == i]
        return member_data_points.mean(axis=0)
    new_centroids = Parallel(n_jobs=-1)(delayed(compute_centroid)(i) for i in range(k))
...
def kmeans(data, k, initial_centroids, maxiter=500, record_heterogeneity=None, verbose=False):
    ...
    for itr in range(maxiter):
        ...
        centroids = revise_centroids(data, k, cluster_assignment)  # [G12 applied here]
