# === Applied G1: Assign repeated expressions to variables
# Comments mark every change related to G1.
"""
...
def main(
    ...
    for _ in range(iterations_num):
        ...
        for _ in range(ants_num):
            ...
            first_city_key = next(iter(cities.keys()))  # APPLIED G1: cache repeated next(iter(...)) for keys
            first_city_val = next(iter(cities.values()))  # APPLIED G1: cache repeated next(iter(...)) for values
            current_city = {first_city_key: first_city_val}  # APPLIED G1: reuse cached values
            del unvisited_cities[first_city_key]  # APPLIED G1: reuse cached key
            ant_route = [first_city_key]  # APPLIED G1: reuse cached key
...
def pheromone_update(
    ...
    KeyError: 0
    """
    cities_len = len(cities)  # APPLIED G1: cache repeated len(cities)
    for a in range(cities_len):  # APPLIED G1: use cached length; update pheromone evaporation
                for b in range(cities_len):  # APPLIED G1: use cached length
                    pheromone[a][b] *= pheromone_evaporation
    for ant_route in ants_route:
        total_distance = 0.0
        steps = len(ant_route) - 1  # APPLIED G1: cache len(ant_route)-1
        for i in range(steps):  # APPLIED G1: use cached steps to avoid recomputation; calculate total distance
            total_distance += distance(cities[ant_route[i]], cities[ant_route[i + 1]])
        delta_pheromone = q / total_distance
        steps = len(ant_route) - 1  # APPLIED G1: cache len(ant_route)-1
        for i in range(steps):  # APPLIED G1: use cached steps; update pheromones
