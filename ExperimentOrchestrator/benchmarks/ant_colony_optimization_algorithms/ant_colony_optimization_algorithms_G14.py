# === Applied G14: Cache distances shared by all workers using lru_cache
# Comments mark every change related to G14.
"""
...
import copy
import random
from functools import lru_cache  # APPLIED G14
import math  # APPLIED G14: used in cached distance
...
def pheromone_update(
    ...
    def _dist_cached(a_idx, b_idx):  # APPLIED G14: cache distances shared across ants
        ax, ay = cities[a_idx]
        bx, by = cities[b_idx]
    ...
    for ant_route in ants_route:
        ...
        for i in range(len(ant_route) - 1):  # Calculate total distance
            total_distance += _dist_cached(ant_route[i], ant_route[i + 1])  # APPLIED G14
